# 文件: datasets/SID_dict_dataset.py (Jittor最终完整版)
...
class BaseDictSet(Dataset):
    def __init__(self, data_path, image_list_file, patch_size=None, split='train', load_npy=True, repeat=1,
        ...
        self.set_attrs(total_len=len(self.img_info) * self.repeat)
    ...
    def __getitem__(self, index):
        ...
        if self.patch_size:
            ...
            if self.split == 'train':
                yy = np.random.randint(0, (H - patch_size) // self.block_size)
                xx = np.random.randint(0, (W - patch_size) // self.block_size)
            else:
            ...
            if self.h_flip and np.random.randint(0, 2) == 1 and self.split == 'train':
                input_patch = np.flip(input_patch, axis=2)
                gt_patch = np.flip(gt_patch, axis=2)
                clean_raw_patch = np.flip(clean_raw_patch, axis=2)
            if self.v_flip and np.random.randint(0, 2) == 1 and self.split == 'train':
                input_patch = np.flip(input_patch, axis=1)
                gt_patch = np.flip(gt_patch, axis=1)
                clean_raw_patch = np.flip(clean_raw_patch, axis=1)
            if self.transpose and np.random.randint(0, 2) == 1 and self.split == 'train':
        ...
        clean_rgb = jt.array(clean_rgb)
        clean_raw = jt.array(clean_raw)
...
class SonyDictSet(BaseDictSet):
    def __init__(self, data_path, image_list_file, patch_size=None, split='train', load_npy=True, repeat=1,
        ...
        super().__init__(
            data_path=data_path, image_list_file=image_list_file, patch_size=patch_size,
            split=split, load_npy=load_npy, repeat=repeat, raw_ext=raw_ext,
            max_samples=max_samples, max_clip=max_clip, min_clip=min_clip,
            only_00=only_00, transpose=transpose, h_flip=h_flip, v_flip=v_flip,
            rotation=rotation, ratio=ratio, batch_size=batch_size, shuffle=shuffle,
            num_workers=num_workers, pin_memory=pin_memory, drop_last=drop_last, **kwargs
        )
...
class FujiDictSet(BaseDictSet):
    def __init__(self, data_path, image_list_file, patch_size=None, split='train', load_npy=True, repeat=1,
                 raw_ext='RAF', max_samples=None, max_clip=1.0, min_clip=None, only_00=False,
                 transpose=True, h_flip=True, v_flip=True, rotation=False, ratio=True,
                 batch_size=1, shuffle=False, num_workers=0, pin_memory=False, drop_last=False, **kwargs):
    ...
    def _pack_raw(self, raw, hh=None, ww=None):
        ...
        if self.patch_size is None:
            ...
            h1, w1 = hh * 3, ww * 3
            h2, w2 = h1 + self.patch_size, w1 + self.patch_size
