...
import jittor as jt # Jittor改动：导入jittor

def set_random_seed(seed):
    # Jittor改动：将torch的seed函数替换为jittor的
    jt.seed(seed)
    np.random.seed(seed)
    random.seed(seed)

#
# --- 从这里开始，以下所有函数都与框架无关，无需任何改动 ---
#
...
def merge_from_base(cfg, cfg_path):
    ...
    if 'base' in cfg:
        ...
        for base_cfg_path in cfg['base']:
            ...
            _merge_a_into_b(base_cfg, base_cfg_total)
        
        _merge_a_into_b(cfg, base_cfg_total)
        return base_cfg_total
    return cfg
